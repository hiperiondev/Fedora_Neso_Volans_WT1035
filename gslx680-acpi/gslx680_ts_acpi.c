...
release:
	...
	printk("Silead Touscreen - x_max / y_max / multi_touches : %d / %d / %d \n",ts->x_max,ts->y_max,ts->multi_touches);
	printk("Silead Touscreen - soft_tracking / x_rev / y_rev / xy_swap : %d / %d / %d / %d \n",ts->soft_tracking,ts->x_reversed,ts->y_reversed,ts->xy_swapped);
	printk("Silead Touscreen - jitter / deadzone : %d / %d \n",ts->jitter,ts->deadzone); 
	return 0;
}
